	...
	//리스트(검색O,페이징 O)
		...
		//////////////////////////////////////////////////
			...
			*/
			...
			//startRowNo, listCnt Map으로 묶는다. 일다하고
			Map<String, Object>limitMap = new HashMap<String, Object>();
			limitMap.put("startRowNo", startRowNo);
			limitMap.put("listCnt", listCnt);
			limitMap.put("keyword", keyword);//별명 오타 조심
			
			//dao에 전달해서 현재페이지의 리스트 10개를 받는다. dao한테 갖다달라 하기
			...
			//0.2 -->1
			//현재페이지 /갯수 올림처리(1/5)*5 -->0.2-->1 0.2(1)*5 => 5
			//(2/5)*5 -->0.4(1)*5 =>5
			//(3/5)*5 -->0.6(1)*5 =>5
			//(4/5)*5 -->0.8(1)*5 =>5
			//(5/5)*5 -->1.0(1)*5 =>5
			...
			if (listCnt * endPageBtnNo < totalCnt) {
				//한페이지당글갯수(10) * 마지막버튼번호(5) < 전체글갯수 102개
				next = true;
			}else { //다음화살표가 false일때 마지막 버튼 번호 정확히 계산 
					//187(tatalCnt) --> p19계산 => 187/10 올림 19
			...
			//startPageBtnNo endPageBtnNo prev next List<TboardVo>
